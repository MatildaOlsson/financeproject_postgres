...
public class PostgresTransactionRepository {
    ...
    protected String tableName = "transactionTableFinal";
    ...
    public PostgresTransactionRepository(String url, String user, String password) throws SQLException {
        ...
        try (Statement createTransactionTable = connection.createStatement()) {
            createTransactionTable.execute("CREATE TABLE IF NOT EXISTS " + tableName + " (" + "id SERIAL PRIMARY KEY," + "amount NUMERIC(10,2) NOT NULL," + "currency VARCHAR(15)," + "is_income BOOLEAN NOT NULL," + "date DATE," + "week INT," + "transaction_info VARCHAR(25))");
        }
    ...
    public List<Transaction> filterByYearMonthOrDay(int value, String type) throws SQLException {
        ...
        List<Transaction> transactionList = new ArrayList<>();
        String sql = "SELECT * FROM " + tableName + " WHERE EXTRACT( " + sqlType + " FROM date) = ? ;";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
    ...
    public BigDecimal getSum(boolean isIncome) throws SQLException {
        String sql = "SELECT SUM(amount) FROM " + tableName + " WHERE is_income = ?";
        BigDecimal sum;

        try (PreparedStatement sumStatement = connection.prepareStatement(sql)) {
            sumStatement.setBoolean(1, isIncome);
    ...
    public String getCurrency(int id) throws SQLException {
        String sql = "SELECT currency FROM " + tableName + " WHERE id = ?";
        String currency;
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setInt(1, id);
            ResultSet resultSet = statement.executeQuery();
